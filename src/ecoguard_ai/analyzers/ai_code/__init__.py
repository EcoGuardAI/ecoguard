...
from ecoguard_ai.core.issue import Issue, Fix, Impact
from ecoguard_ai.analyzers.base import BaseAnalyzer, ASTVisitorRule
...
class VerboseCodeRule(ASTVisitorRule):
    ...
    def _is_redundant_nested_if(self, node: ast.If) -> bool:
        ...
        if (len(node.body) == 1 and 
            ...
            not node.body[0].orelse):  # No else clause
            return True
        return False
    ...
    def _has_verbose_return_pattern(self, node: ast.FunctionDef) -> bool:
        ...
        for stmt in node.body:
            if (isinstance(stmt, ast.If) and
                len(stmt.body) == 1 and len(stmt.orelse) == 1 and
                ...
                if (isinstance(body_return.value, ast.Constant) and
                    isinstance(else_return.value, ast.Constant) and
                    isinstance(body_return.value.value, bool) and
                    isinstance(else_return.value.value, bool)):
                    return True
        return False
    ...
    def _check_immediate_return_pattern(self, node: ast.FunctionDef) -> None:
        ...
        if len(node.body) >= 2:
            for i in range(len(node.body) - 1):
                ...
                if (isinstance(stmt1, ast.Assign) and
                    isinstance(stmt2, ast.Return) and
                    len(stmt1.targets) == 1 and
                    isinstance(stmt1.targets[0], ast.Name) and
                    isinstance(stmt2.value, ast.Name) and
                    stmt1.targets[0].id == stmt2.value.id):
...
class DuplicateFunctionRule(ASTVisitorRule):
    ...
    def _are_functions_similar(self, func1: ast.FunctionDef, func2: ast.FunctionDef) -> bool:
        ...
        if (len(func1.body) == len(func2.body) and
            len(func1.args.args) == len(func2.args.args) and
            len(func1.body) > 2):  # Only check functions with some complexity
...
class UnnecessaryTypeChecksRule(ASTVisitorRule):
    ...
    def _is_none_check(self, test: ast.expr) -> bool:
        ...
        if isinstance(test, ast.Compare):
            return any(isinstance(comp, ast.Is) or isinstance(comp, (ast.Eq, ast.NotEq))

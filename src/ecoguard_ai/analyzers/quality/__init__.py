        ...
        for var_name, var_node in self.scopes[-1].items():
            if var_name not in self.used_names[-1] and not var_name.startswith('_'):
                issue = self.create_issue(
                    ...
                    suggested_fix=Fix(
                        description=f"Remove unused variable '{var_name}' or prefix with underscore if intentional",
                        replacement_code=f"# Remove line or rename to _{var_name}"
                    ),
                    impact=Impact(
                        maintainability=-0.5,
            ...
            if name not in self.used_names:
                issue = self.create_issue(
                    ...
                    suggested_fix=Fix(
                        ...
                        replacement_code=f"# Remove import line"
                    ),                        impact=Impact(
                            performance=-0.1,
                            maintainability=-0.2
                        )
...
class FunctionComplexityRule(ASTVisitorRule):
    ...
    def _calculate_complexity(self, node: ast.FunctionDef) -> int:
        ...
        for child in ast.walk(node):
            ...
            if isinstance(child, (ast.If, ast.While, ast.For, ast.ListComp, ast.DictComp, ast.SetComp)):
                complexity += 1

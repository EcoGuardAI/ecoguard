...
def main():
    ...
    if not run_command("pre-commit run --all-files", "Running initial pre-commit checks"):
    ...
    if not Path(".secrets.baseline").exists():
        run_command("detect-secrets scan > .secrets.baseline", "Creating secrets baseline")
    ...
    run_command("ecoguard analyze src/ --format table", "Running EcoGuard AI self-analysis")

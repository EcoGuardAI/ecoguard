...
'''
    ...
    try:
        ...
        if not result.issues:
            print("✅ No issues found! (This is expected since rules aren't implemented yet)")
        else:
            print(f"Found {result.issue_count} issues:")
            
            for issue in result.issues:
                print(f"  Line {issue.line}: {issue.severity.value.upper()} - {issue.message}")
                print(f"    Rule: {issue.rule_id} (Category: {issue.category.value})")
        ...
        custom_analyzer = EcoGuardAnalyzer(custom_config)

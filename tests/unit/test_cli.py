...
from ecoguard_ai.cli import cli, main, _get_severity_color, _display_single_result, _display_project_result
    ...
    def test_analyze_with_valid_file(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
            ...
            try:
                ...
                # Should execute without error (may exit with 0 or 1 depending on issues found)
    ...
    def test_analyze_with_disabled_modules(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
            ...
            try:
                ...
                result = runner.invoke(cli, [
                    'analyze', f.name, 
                    '--no-quality', '--no-security', '--no-green', '--no-ai-code'
                ])
                assert result.exit_code in [0, 1]
            finally:
                Path(f.name).unlink(missing_ok=True)
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
            ...
            try:
                ...
                for format_type in ['table', 'json', 'text']:
                    result = runner.invoke(cli, ['analyze', f.name, '--format', format_type])
                    assert result.exit_code in [0, 1]
